...
대상 장르가 **발표 슬라이드(pptx)·개조식 보고서**면 [`references/deck-supplement.md`](references/deck-supplement.md)를 함께 로드한다. 슬라이드에서는 불릿·숫자 인덱싱·화살표(→)·체언 종결이 정상 관례라 심각도가 달라지고(과잉 교정 방지), 반대로 줄표(—)와 가운뎃점(·) 오용은 더 강한 신호다. 개조식 변환 규칙(명사형 종결·헤더 이원화·가로 흐름 테스트)과 기호 치환표(— → 괄호/⇒/※)도 그 파일에 있다.
